...
public class DislikeCountConfig implements FeaturePropertyProvider {
    ...
    public DislikeCountConfig(long median, double std, String criteria) {
        if (median < 0 || std < 0) {
    ...
    public static DislikeCountConfig.Builder builder() {
    ...
    public static class Builder {
        ...
        public DislikeCountConfig.Builder criteria(String criteria) {
            this.criteria = criteria;
            return this;
        }

        public DislikeCountConfig.Builder median(long median) {
            this.median = median;
            return this;
        }

        public DislikeCountConfig.Builder std(double std) {
            this.std = std;
            return this;
        }

        public DislikeCountConfig build() {
            if(median == null) {
                log.warn("싫어요 수 중앙값이 설정되지 않아 {}으로 설정합니다", DEFAULT_MEDIAN);
                median = DEFAULT_MEDIAN;
            }
            if(std == null) {
                log.warn("싫어요 수 표준편차가 설정되지 않아 {}으로 설정합니다", DEFAULT_HEURISTIC_FACTOR * median);
                std = DEFAULT_HEURISTIC_FACTOR * median;
            }
            if(criteria == null) {
                criteria = DEFAULT_CRITERIA;
            }
            return new DislikeCountConfig(median, std, criteria);
        }
    }
}
