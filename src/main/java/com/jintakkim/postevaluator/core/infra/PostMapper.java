...
import com.jintakkim.postevaluator.core.Feature;
...
public class PostMapper implements RowMapper<Post> {
    ...
    public Post map(ResultSet rs, StatementContext ctx) throws SQLException {
        Map<String, Feature> features = extractFeatures(rs);
        long id = rs.getLong(ID_COLUMN_NAME);
        return new Post(id, features);
    }

    private Map<String, Feature> extractFeatures(ResultSet rs) throws SQLException {
        ...
        for (int i = 1; i <= columnCount; i++) {
            ...
            Feature feature = new Feature(rs.getObject(i));
            features.put(columnName, feature);
        }
        return features;
    }
}
